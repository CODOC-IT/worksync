// Static Pakistan public holiday data for read-only Calendar entries. Fixed-date holidays
// (Gregorian, same day every year) are computed for any year on request. Islamic (Hijri lunar
// calendar) holidays shift roughly 11 days earlier each Gregorian year and are only knowable in
// advance via the officially-published civil calendar / moon-sighting announcement, so they're
// looked up from a per-year table covering a reasonable navigation window around "now" -- extend
// LUNAR_HOLIDAYS with more years as official/estimated dates become available. Years outside
// that table simply show no lunar holiday; fixed-date holidays are unaffected.
...
// Published/estimated Gregorian dates for Pakistan; the actual date is confirmed close to each
// occasion by moon sighting and may shift by a day.
