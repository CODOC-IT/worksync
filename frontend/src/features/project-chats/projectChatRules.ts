...
export const filterDiscussions = (threads: DiscussionThread[], filters: DiscussionFilters, currentUserId: string, projectNames: Record<string, string>, taskNames: Record<string, string>, teamNames: Record<string, string> = {}) => {
  const query = filters.search.trim().toLowerCase();
  return threads.filter((thread) => {
    const comments = thread.comments.map((comment) => comment.body).join(' ').toLowerCase();
    const matchesSearch = !query || [thread.title, comments, projectNames[thread.projectId] || '', taskNames[thread.taskId || ''] || '', teamNames[thread.teamId || ''] || ''].join(' ').toLowerCase().includes(query);
    const matchesDate = (!filters.from || thread.createdAt >= filters.from) && (!filters.to || thread.createdAt <= `${filters.to}T23:59:59.999Z`);
    return matchesSearch && (!filters.projectId || thread.projectId === filters.projectId) && (!filters.taskId || thread.taskId === filters.taskId) && (!filters.teamId || thread.teamId === filters.teamId) && (!filters.type || thread.type === filters.type) && (!filters.authorId || thread.creatorId === filters.authorId) && (!filters.mineOnly || thread.creatorId === currentUserId) && (!filters.mentionedOnly || thread.comments.some((comment) => comment.mentionIds.includes(currentUserId))) && matchesDate;
