  ...
  useEffect(() => {
    ...
    const loadFilteredActivities = async () => {
      ...
      try {
        ...
        // Fetch every page inside the window (capped at MAX_ACTIVITIES) so the widget shows
        // the full period instead of only the newest page. A single capped request would
        // otherwise surface the same handful of events for every filter whenever one day
        // produces more activity than the page size.
        ...
        const maxPages = Math.min(
          ...
          Math.ceil(MAX_ACTIVITIES / PAGE_SIZE),
        );
        for (let page = 2; page <= maxPages && collected.length < total; page++) {
          if (cancelled) return;
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="flex-1 min-w-[300px] max-w-full">
          <div className="glass-panel h-104 p-3 border border-cyan-500/20 overflow-y-auto">
        ...
        {/* Activity Log */}
        <div className="flex-1 min-w-[300px] max-w-full">
          <div className="glass-panel p-3 border border-purple-500/20 h-104 overflow-y-auto flex flex-col">
