...
const CATEGORY_INSTRUCTIONS: Record<string, string> = {
  ...
  '2. Every Task in the Project: For each task, include and analyze the task number, title, full description, current status, priority, assignee, due date, dependencies, **full status change history/timeline** (including when each status change occurred, who changed it, and their note/reason), and any discussion/progress updates available in the project data. Explain the significance of each task\'s current status, reconstruct the task\'s lifecycle, and identify whether it is completed, progressing normally, at risk, pending, or blocked.\n' +
